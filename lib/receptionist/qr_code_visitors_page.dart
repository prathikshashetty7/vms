...
class _QRCodeVisitorsPageState extends State<QRCodeVisitorsPage> {
        ...
        builder: (context, snapshot) {
          ...
          return ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              final docId = qrCodeDocs[index].id;
              ...
              return Container(
                ...
                child: Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      CircleAvatar(
                            backgroundColor: Color(0xFF6CA4FE).withOpacity(0.13),
                        radius: 26,
  ...
  void _showEditVisitorSheet(BuildContext context, Map<String, dynamic> data, String docId) {
    showModalBottomSheet(
      ...
      builder: (context) => _EditVisitorForm(data: data, docId: docId, collection: 'manual_registrations'),
    );
  }
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              await FirebaseFirestore.instance.collection('manual_registrations').doc(docId).delete();
              if (context.mounted) {
    ...
    return Padding(
      ...
      child: StreamBuilder<QuerySnapshot>(
        stream: FirebaseFirestore.instance
            ...
            .where('group', isEqualTo: 'qr_code')  // Filter for QR code passes only
            .snapshots(),
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            return const Center(child: CircularProgressIndicator());
              ...
              Expanded(
                child: ListView.builder(
                  itemCount: sortedDocs.where((doc) {
                    final data = doc.data() as Map<String, dynamic>;
                    return (data['visitorName']?.toString().toLowerCase().contains(searchQuery) == true ||
...
class _VisitorDetailsDialog extends StatelessWidget {
  ...
  Future<int> _generateUniquePassNo() async {
    ...
    // Try up to 5 times with different random numbers
    for (int attempt = 0; attempt < 5; attempt++) {
      final passNo = 1000 + random.nextInt(9000); // 4-digit number
      
      try {
        final query = await passesRef
            .where('pass_no', isEqualTo: passNo)
            .limit(1)
            .get()
            .timeout(const Duration(seconds: 2)); // Add timeout
        
        if (query.docs.isEmpty) {
          return passNo; // Found unique number
        }
      } catch (e) {
                ...
                Center(
                  child: ElevatedButton.icon(
                    ...
                    onPressed: () async {
                      ...
                      try {
                        // Generate unique pass number with timeout
                        ...
                        // Add pass to collection with timeout
                        ...
                        if (context.mounted) {
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text('Pass generated successfully (Pass #$passNo)'),
                              backgroundColor: Colors.green,
                        ...
                        if (context.mounted) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text('Error generating pass: ${e.toString()}'),
                  ...
                  onPressed: () async {
                    try {
                      ...
                      try {
                        ...
                        if (existingDocs.docs.isEmpty) {
                          ...
                          await FirebaseFirestore.instance.collection('checked_in_out').add({
                            ...
                            'check_in_date': _formatDateOnly(DateTime.now()),
                            'status': 'Checked In',
                            'pass_id': passId,
                            'created_at': FieldValue.serverTimestamp(),
                          });
                      ...
                      if (context.mounted) {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text('Pass printed and visitor checked in successfully!'),
                            backgroundColor: Colors.green,
                      ...
                      if (context.mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text('Error printing pass: $e'),
                            backgroundColor: Colors.red,
                            duration: const Duration(seconds: 3),
                          ),
                        );
